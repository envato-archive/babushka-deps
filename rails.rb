...
dep 'bundler installed' do #DONE
  ...
  meet { in_dir(var(:rails_root)) {
    bundle_command = "bundle install --without test,cucumber,development #{'--local' if var(:bundler_installed_locally).to_s =~ /^y/} #{'--path vendor/bundle' if var(:bundle_into_vendor).to_s =~ /^y/}"
